* petalinux-docker
:PROPERTIES:
:CUSTOM_ID: petalinux-docker
:END:

* 前期准备

需要安装docker，可参考链接：[[https://cloud.tencent.com/document/product/213/46000][腾讯云服务器搭建Docker]]、[[https://ynhugo.github.io/p/云服务器搭建docker/#headline-6][Docker换源]]、[[https://github.com/DaoCloud/public-image-mirror?tab=readme-ov-file][public-image-mirror]] 

** 1. 拷贝petalinux-v2022.1-04191534-installer.run到这个目录中

** 2. 构建与安装petalinux2022.1
#+begin_src bash
# docker build --build-arg PETA_VERSION=2022.1 --build-arg PETA_RUN_FILE=petalinux-v2022.1-04191534-installer.run -t petalinux:2022.1 .

# 配置阿里云安装源
docker build --build-arg UBUNTU_MIRROR=mirrors.aliyun.com --build-arg PETA_VERSION=2022.1 --build-arg PETA_RUN_FILE=petalinux-v2022.1-04191534-installer.run -t petalinux:2022.1 .
#+end_src

** 完成构建后运行
#+begin_src bash
# docker run -ti --rm -e DISPLAY=$DISPLAY --net="host" -v /tmp/.X11-unix:/tmp/.X11-unix -v $HOME/.Xauthority:/home/vivado/.Xauthority -v $HOME/Projects:/home/vivado/project  petalinux:2022.1 /bin/bash
mkdir -p Projects; cd Projects
docker run -ti --rm -e DISPLAY=$DISPLAY --net="host" -v /tmp/.X11-unix:/tmp/.X11-unix -v $HOME/.Xauthority:/home/vivado/.Xauthority -v ./:/home/vivado/project  petalinux:2022.1 /bin/bash
#+end_src
这个 Docker 命令的选项解释：
|-----------------------------------------------+-----------------------------------------|
| 选项                                          | 作用                                    |
|-----------------------------------------------+-----------------------------------------|
| docker run                                    | 从镜像创建并运行容器                    |
|-----------------------------------------------+-----------------------------------------|
| -t                                            | 分配伪终端（TTY），支持交互式终端       |
|-----------------------------------------------+-----------------------------------------|
| -i                                            | 保持 STDIN 打开，实现交互式输入         |
|-----------------------------------------------+-----------------------------------------|
| --rm                                          | 容器退出后自动删除                      |
|-----------------------------------------------+-----------------------------------------|
| -e DISPLAY=$DISPLAY                           | 传递 X11 显示环境变量，用于图形界面转发 |
|-----------------------------------------------+-----------------------------------------|
| --net="host"                                  | 使用主机网络栈（容器共享主机网络）      |
|-----------------------------------------------+-----------------------------------------|
| -v /tmp/.X11-unix:/tmp/.X11-unix                | 挂载 X11 Unix 套接字，用于图形程序显示  |
|-----------------------------------------------+-----------------------------------------|
| -v $HOME/.Xauthority:/home/vivado/.Xauthority | 挂载 X11 认证文件，解决权限问题         |
|-----------------------------------------------+-----------------------------------------|
| -v $HOME/Projects:/home/vivado/project        | 挂载项目目录到容器内                    |
|-----------------------------------------------+-----------------------------------------|
| petalinux:2022.1                              | 使用的镜像名称                          |
|-----------------------------------------------+-----------------------------------------|
| /bin/bash                                     | 容器启动后执行的命令                    |
|-----------------------------------------------+-----------------------------------------|

** 4. auto.sh的使用
auto.sh是自动配置离线编译的脚本，需要先在脚本正确配置离线编译环境的路径，然后在创建完petalinux工程并导入硬件描述文件后运行该脚本，示例：
#+begin_src bash
./auto.sh zynq
# 或
./auto.sh zynqmp
#+end_src

** 5. petalinux使用流程
#+begin_src bash
bsp_name="xilinx-zcu102-v2022.1-04191534.bsp"
prj_index=3
prj_name="prj_"

sudo chown -R vivado:vivado ../$(basename $PWD)

# 如果不指定工程名称，会以bsp文件命名工程
# 从BSP中创建工程
# petalinux-create -t project -s $bsp_name
petalinux-create -t project -s $bsp_name -n $prj_name$prj_index
# 不从BSP中创建工程
# petalinux-create 命令用于创建新的PetaLinux 工程：
# petalinux-create -t project --template zynqmp -n $prj_name$prj_index

cd $prj_name$prj_index

# 默认设置是启用 WebTalk 选项，向赛灵思反馈工具使用统计数据。可以运行 petalinux-util --webtalk 命令，关闭 Web Talk 功能
petalinux-util --webtalk off

../auto.sh zynqmp

petalinux-config --get-hw-description hardware/xilinx-zcu102-2022.1/outputs/project_1.xsa

# 增加time选项是为了查看最终编译花了多长时间
time petalinux-build

petalinux-package --boot --fsbl --fpga --u-boot --force
#+end_src

** 以下可忽略：

使用petalinux如果不想被收集使用数据，可以在petalinux的项目根目录中执行`petalinux-util --webtalk off`

WebTalk 是 Xilinx/AMD 的遥测功能，会收集你使用 PetaLinux 工具的匿名数据并发送给 AMD。

关闭它的原因：
1. 隐私 ― 不想被收集使用数据
2. 避免构建错误 ― WebTalk 需要联网，在很多环境下会失败，报错如：
      webtalk failed:Failed to get PetaLinux usage statistics!
3. 老旧功能 ― 从 PetaLinux 2022.2 开始，WebTalk 已被 AMD 移除

你用的 2022.1 版本还有这个功能，所以建议关闭。如果你确定不会遇到网络问题，也可以不关，不会影响核心功能。

注意：你现在已经在容器内了，运行 petalinux-util --webtalk off 需要在项目目录内执行，而不是全局设置。
